...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (file instanceof File && file.size > 0) {
    try {
      update.image_storage_path = await uploadSiteImage("attivita", file);
    } catch (err) {
      console.error("[admin/activities/:id] errore upload immagine", err);
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (activity?.image_storage_path) await deleteSiteImage(activity.image_storage_path);
